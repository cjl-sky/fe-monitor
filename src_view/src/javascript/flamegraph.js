...
function find_child(parent, name, attr) {
  ...
  for (let i = 0; i < children.length; i++) {
    if (children[i].tagName == name) return attr != undefined ? children[i].attributes[attr].value : children[i];
  }
  return;
}
...
function g_to_func(e) {
  ...
  if (func != null) func = func.replace(/ .*/, '');
  return func;
}
...
const contextify = (function() {
  ...
  function contextify(parsed, opts) {
    ...
    function processNode(node, index) {
      ...
      if (chars >= 3) {
        // enough room to display function name?
        showText = true;
...
function set_parent_inf() {
  this.showBigPic ? this.$emit('hidePic') : this.$emit('changPic');
}
